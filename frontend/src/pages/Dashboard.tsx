...
export default function Dashboard() {
  ...
  const tierColors: Record<string, string> = {
    'I': 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300',
    'II': 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400',
    'III': 'bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-400',
    'IV': 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400',
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">Overview of your submittal review projects</p>
      </div>
          ...
          {statCards.map((s) => (
            ...
            >
              ...
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-slate-400">{s.label}</p>
                  <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-slate-100 tracking-tight">{s.value}</p>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {stats.submittals_by_status && Object.keys(stats.submittals_by_status).length > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-4">Submittals by Status</h3>
                  ...
                  return (
                    <div key={status}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-gray-600 dark:text-slate-400 capitalize">{status.replace('_', ' ')}</span>
                        <span className="font-semibold dark:text-slate-300">{count}</span>
                      </div>
          ...
          {stats.comments_by_severity && Object.keys(stats.comments_by_severity).length > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-4">Open Comments by Severity</h3>
              <div className="space-y-3">
                {Object.entries(stats.comments_by_severity).map(([severity, count]: [string, any]) => {
                  ...
                  return (
                    <div key={severity}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="text-gray-600 dark:text-slate-400 capitalize">{severity}</span>
                        <span className="font-semibold dark:text-slate-300">{count}</span>
                      </div>
                      <div className="h-2 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
      ...
      <div>
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-slate-100">Projects</h2>
        ...
        {showCreate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {/* Modal */}
            <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
              {/* Modal Header */}
              <div className="px-6 py-5 border-b border-gray-100 dark:border-slate-700 bg-gray-50/80 dark:bg-slate-800/80">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-slate-100">Create New Project</h3>
                    <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Set up a new submittal review project</p>
                  </div>
                  <button
                    onClick={() => setShowCreate(false)}
                    className="text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300 rounded-lg p-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
                  >
              ...
              <form onSubmit={handleCreate} className="p-6 space-y-5">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full border border-gray-300 dark:border-slate-600 rounded-lg px-3.5 py-2.5 text-sm text-gray-900 dark:text-slate-200 bg-white dark:bg-slate-700 placeholder:text-gray-400 dark:placeholder:text-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow"
                  />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Client</label>
                    <input
                      placeholder="e.g., ABB"
                      value={form.client}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Location</label>
                    <input
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Redundancy Tier</label>
                  <select
                    value={form.tier_level}
                    onChange={(e) => setForm({ ...form, tier_level: e.target.value })}
                    className="w-full border border-gray-300 dark:border-slate-600 rounded-lg px-3.5 py-2.5 text-sm text-gray-900 dark:text-slate-200 bg-white dark:bg-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow appearance-none cursor-pointer"
                  >
        ...
        <div className="grid gap-4">
          {projects.map((project) => (
            <Link
              ...
              className="group bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 hover:shadow-lg hover:border-gray-300 dark:hover:border-slate-600 transition-all duration-200"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    ...
                    {project.tier_level && (
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${tierColors[project.tier_level] || 'bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-300'}`}>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-gray-500 dark:text-slate-400">
                    {project.client && (
                      <span className="inline-flex items-center gap-1.5">
                        <svg className="w-3.5 h-3.5 text-gray-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                  ...
                  {project.description && (
                    <p className="text-sm text-gray-400 dark:text-slate-500 mt-2 line-clamp-1">{project.description}</p>
                  )}
                </div>

                <div className="flex items-center gap-4 flex-shrink-0">
                  <div className="text-center px-4 py-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                    <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{project.submittal_count}</div>
                    <div className="text-xs font-medium text-blue-500 dark:text-blue-500 mt-0.5">
                      {project.submittal_count === 1 ? 'submittal' : 'submittals'}
                    </div>
                  </div>
                  <svg className="w-5 h-5 text-gray-300 dark:text-slate-600 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          ...
          {projects.length === 0 && (
            <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-xl border border-dashed border-gray-300 dark:border-slate-600">
              ...
              <h3 className="text-base font-semibold text-gray-900 dark:text-slate-100">No projects yet</h3>
              <p className="mt-1.5 text-sm text-gray-500 dark:text-slate-400 max-w-sm mx-auto">
