...
export default function RFITracker() {
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      ...
      case 'sent': return 'bg-blue-50 text-blue-600 ring-1 ring-blue-200';
      case 'responded': return 'bg-amber-50 text-amber-600 ring-1 ring-amber-200';
      case 'closed': return 'bg-emerald-50 text-emerald-600 ring-1 ring-emerald-200';
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">RFI Tracker</h1>
        <p className="mt-1 text-sm text-slate-500">Track Requests for Information across all submittals</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs text-slate-400 ml-auto">{rfis.length} results</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    <span className="text-sm font-bold text-slate-900">{rfi.rfi_number}</span>
                  ...
                  <p className="text-sm font-semibold text-slate-800 mb-1">{rfi.subject}</p>
                  {rfi.description && (
                    <p className="text-sm text-slate-600 leading-relaxed">{rfi.description}</p>
                  )}
                  {rfi.response && (
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    {rfi.project_name && (
                      <span className="text-xs text-slate-400">{rfi.project_name}</span>
                    )}
                    {rfi.submittal_title && (
                      <span className="text-xs text-slate-400">{rfi.submittal_title}</span>
                    )}
                    <span className="text-xs text-slate-400">
        ...
        {!loading && rfis.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-sm font-semibold text-slate-700">No RFIs found</h3>
            <p className="text-sm text-slate-400 mt-1">RFIs will appear here when generated from submittal reviews</p>
          </div>
        )}
      </div>
    </div>
